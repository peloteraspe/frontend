...
export default function LandingGrowthBlocks() {
  return (
    <div className="w-full">
      ...
      {/* ── Para jugadoras ─────────────────────────────────────────── */}
      <section className="w-full border-t border-slate-100 bg-white" id="para-jugadoras">
        <div className={inner}>
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.9fr)] lg:items-start">
            ...
            <div className="flex flex-col divide-y divide-slate-100">
              {playerBenefits.map((item, i) => (
                <div key={item.title} className="flex gap-5 py-5 first:pt-0 last:pb-0">
                  <span className="mt-0.5 w-8 shrink-0 font-eastman-extrabold text-2xl leading-none text-mulberry select-none">
      ...
      <section className="w-full bg-[#f4ecf9]" id="para-administradoras">
        <div className={inner}>
          <div className="max-w-[34rem]">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-mulberry/75">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {adminBenefits.map((item, i) => (
              <article
                key={item.title}
                className="rounded-2xl bg-white px-5 py-5 shadow-[0_1px_4px_rgba(84,8,111,0.07)]"
              >
          ...
          <div className="mt-7">
            <OrganizerEntryLink className="inline-flex h-11 items-center rounded-full bg-mulberry px-5 text-sm font-semibold text-white transition hover:bg-[#470760]">
      ...
      {/* ── Patrocinios ────────────────────────────────────────────── */}
      <section className="w-full bg-mulberry" id="patrocinios">
        <div className={inner}>
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            ...
            <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
              <Link
                href="/patrocinios"
                className="inline-flex h-11 items-center justify-center rounded-full bg-white px-6 text-sm font-semibold text-mulberry transition hover:bg-slate-100"
              >
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
            ...
            <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
              ...
              <Link
                ...
                className="inline-flex h-11 items-center justify-center rounded-full border border-slate-900/25 px-6 text-sm font-semibold text-slate-900 transition hover:bg-slate-900/8"
              >
                Crear mi cuenta
              </Link>
            </div>
