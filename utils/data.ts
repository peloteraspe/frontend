...
const hasNumberKey = (v: OptionSelect): v is OptionSelect & { key: number } =>
  typeof v.key === 'number';

export const playerPositionsFormatted = (options: OptionSelect[]): number[] => {
  return options.filter(hasNumberKey).map((v) => v.key);
};

export const levelFormatted = (option: OptionSelect): number | null => {
  return typeof option.key === 'number' ? option.key : null;
};
