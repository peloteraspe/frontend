  ...
  export default function Login({
    ...
    const handleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      setButtonText('Cargando...'); // Update button text to indicate loading state

      const formData = new FormData(e.currentTarget);
      const email = formData.get('email') as string;

      if (!emailValue) { // Verifica si el campo de correo electrónico está vacío
      ...
      try {
        ...
        toast.success('¡Enlace enviado! Revisa tu correo para ingresar a tu cuenta'); // Display success toast
        setButtonText('Enlace mágico enviado'); // Update button text after successful submission
      } catch (error) {
        console.error('Error during sign in:', error);
        toast.error('Hubo un error al enviar el enlace. Por favor, inténtalo de nuevo.'); // Display error toast
    ...
    const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setEmailError(false); // Restablecer el estado de error del correo electrónico a falso
      ...
      <div className="flex flex-col justify-center items-center w-full gap-8 h-[calc(100vh-6rem)]">
        ...
        <div className="flex flex-col items-center">
          ...
          >
            ...
            />
            <div className="flex flex-col items-center">
              <ButtonWrapper type="submit" disabled={emailError}>{buttonText}</ButtonWrapper> {/* Use buttonText state for button text */}
            </div>
          </form>
        </div>
